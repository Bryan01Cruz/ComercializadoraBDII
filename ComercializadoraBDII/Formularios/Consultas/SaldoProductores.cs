...
{
    public partial class SaldoProductores : Form
    {
        public SaldoProductores()
        {
            InitializeComponent();
        }
        ...
        public DataTable CargarInventario(string filtro)
        {
            ...
            try
            {
                string sql = @"
                SELECT * 
                FROM vw_SaldoPendienteProductores 
                WHERE Agricultor LIKE @filtro OR AgricultorID LIKE @filtro";
        ...
        private void SaldoProductores_Load(object sender, EventArgs e)
        {
            try
            {
